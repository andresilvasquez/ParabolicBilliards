...
for ang in angles:
    ...
    D = np.loadtxt("resultados/density_0.005000_0.000000.dat")
    P = np.loadtxt("resultados/phase_0.005000_0.000000.dat")
    N = D.shape[0]
    x = np.linspace(-8, 8, N)
    y = np.linspace(-8, 8, N)
    X, Y = np.meshgrid(x, y)
    fig.suptitle(rf"Billar ({xi0},{eta0}) | $k^2$ = 0.0005 | $\beta$ = {ang}.0°")
    ...
    def update(k2):
        D = np.loadtxt(f"resultados/density_{k2:.3f}000_{ang}.000000.dat")
        P = np.loadtxt(f"resultados/phase_{k2:.3f}000_{ang}.000000.dat")
        ...
        fig.suptitle(f'Billar({xi0},{eta0}) | $k^2$={k2:.2f} | Ángulo: {ang:.1f}°')

        return im1, im2

    ani = animation.FuncAnimation(fig, update, frames=K2, blit=True, interval=50, repeat=False)

    writer = FFMpegWriter(fps=180, metadata=dict(title=rf'Simulacion de Billar $\beta$={ang:.1f}'), codec='mpeg4', extra_args=['-q:v', '2'])
    ani.save(f"Billar_ang_{ang:.1f}.mp4", writer=writer)

    plt.close()
    print(f"Video creado: ang = {ang}")
